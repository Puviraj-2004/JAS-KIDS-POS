...
const yesterday = () => { const date = new Date(); date.setDate(date.getDate() - 1); return dateInput(date); };
const weekStart = () => { const date = new Date(); const day = date.getDay() || 7; date.setDate(date.getDate() - day + 1); return dateInput(date); };
...
export function SalesHistory() {
  ...
  return <main className={styles.page}>
    <div className={styles.shell}>
      ...
      <nav className="record-tabs" aria-label="History type"><Link href="/pos/sales" aria-current="page">Sales</Link><Link href="/pos/bookings">Bookings</Link></nav>
...
function SaleModal({ sale, showCashier, close }: { sale: Sale; showCashier: boolean; close: () => void }) {
  ...
  return <div className={styles.modalBackdrop} role="presentation"><section className={styles.modal} role="dialog" aria-modal="true" aria-labelledby="sale-modal-title"><div className={styles.modalHeader}><div><p>SALE DETAILS</p><h2 id="sale-modal-title">{sale.sale_no}</h2><span>{new Date(sale.created_at).toLocaleString("en-GB", { dateStyle: "medium", timeStyle: "short" })}</span></div><button type="button" onClick={close} aria-label="Close sale details">×</button></div><div className={styles.detailGrid}><section className={styles.detailCard}><h3>Sale information</h3><dl><div><dt>Receipt</dt><dd>{receiptNo}</dd></div><div><dt>Status</dt><dd><span className={sale.status === "CANCELLED" ? styles.cancelled : styles.complete}>{sale.status === "CANCELLED" ? "Cancelled" : "Completed"}</span></dd></div><div><dt>Payment</dt><dd>{sale.payment_method}</dd></div>{showCashier && <div><dt>Cashier</dt><dd>{sale.cashier.name}</dd></div>}</dl>{sale.cancellation_reason && <p className={styles.cancelNote}><strong>Cancellation:</strong> {sale.cancellation_reason}</p>}</section><section className={styles.detailCard}><h3>Payment summary</h3><dl><div><dt>Subtotal</dt><dd>{money.format(Number(sale.subtotal))}</dd></div><div><dt>Discount</dt><dd>−{money.format(Number(sale.discount))}</dd></div><div className={styles.totalLine}><dt>Total</dt><dd>{money.format(Number(sale.total))}</dd></div><div><dt>Amount received</dt><dd>{money.format(Number(sale.amount_received))}</dd></div><div><dt>Change given</dt><dd>{money.format(Number(sale.change_given))}</dd></div></dl></section></div><section className={styles.detailCard}><h3>Items</h3><div className={styles.modalTable}><table><thead><tr><th>Product</th><th data-numeric>Qty</th><th data-numeric>Unit price</th><th data-numeric>Unit cost</th><th data-numeric>Total</th></tr></thead><tbody>{sale.items.map(item => <tr key={item.id}><td>{item.product_name}</td><td data-numeric>{Number(item.quantity)}</td><td data-numeric>{money.format(Number(item.unit_price))}</td><td data-numeric>{money.format(Number(item.unit_cost))}</td><td data-numeric>{money.format(Number(item.line_total))}</td></tr>)}</tbody></table></div></section><div className={styles.modalActions}><button type="button" onClick={close}>Close</button><button className={styles.printButton} type="button" onClick={() => void print()}><Printer size={17} aria-hidden="true" />Print receipt</button></div><SaleReceipt receiptNo={receiptNo} saleNo={sale.sale_no} status={sale.status} branchName={sale.branch.name} branchAddress={sale.branch.address} branchPhone={sale.branch.phone} servedBy={sale.cashier.name} issuedAt={sale.created_at} items={sale.items} subtotal={sale.subtotal} discount={sale.discount} total={sale.total} amountReceived={sale.amount_received} changeGiven={sale.change_given} paymentMethod={sale.payment_method} /></section></div>;
}
