"use client";
import { ArrowLeft, ArrowRight, Minus, Plus, Search, ShoppingCart, Trash2 } from "lucide-react";
...
export function SalesTerminal() {
  ...
  const load = useCallback(async (selected?: string) => {
    ...
    try {
      ...
      setDiscount("0"); setAmountReceived(""); setCategory("ALL"); setMobileCart(false);
  ...
  const lines = useMemo(() => data?.products.filter(product => cart[product.id]).map(product => ({ product, quantity: cart[product.id], lineTotal: Number(product.selling_price) * cart[product.id] })) ?? [], [data, cart]);
  ...
  async function clearCart() {
    ...
    setCart({}); setDiscount("0"); setAmountReceived(""); setError("");
  }
  async function checkout() {
    ...
    try {
      const response = await fetch("/api/pos/sales", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ branch_id: branchId, items: lines.map(line => ({ stock_batch_id: line.product.stock_batch_id, quantity: line.quantity })), discount: discountNumber, amount_received: received, payment_method: paymentMethod, idempotency_key: crypto.randomUUID() }) });
      const payload = await response.json();
  ...
  return <main className={styles.page}><div className={styles.shell}>
    <header className={styles.title}><div><p>COUNTER</p><h1>New sale</h1><span>Select products, review the cart, then collect payment.</span></div>{data && data.branches.length > 1 && <label>Sales branch<select value={branchId} disabled={loading || submitting} onChange={event => void changeBranch(event.target.value)}>{data.branches.map(branch => <option value={branch.id} key={branch.id}>{branch.name}</option>)}</select></label>}</header>
    ...
    <div className={styles.terminal + (mobileCart ? " " + styles.reviewing : "")}>
      ...
      <aside className={styles.cart} aria-label="Current sale">
        ...
        <form className={styles.checkout} onSubmit={event => { event.preventDefault(); void checkout(); }}>
          <fieldset disabled={!lines.length || submitting || loading}><div className={styles.paymentFields}><label>Discount · LKR<input type="number" min="0" max={subtotal} step="0.01" value={discount} onChange={event => setDiscount(event.target.value)}/></label><label>Payment method<select value={paymentMethod} onChange={event => setPaymentMethod(event.target.value)}><option value="CASH">Cash</option><option value="CARD">Card</option><option value="UPI">UPI</option><option value="ONLINE">Online</option></select></label></div><label>Amount received · LKR<input type="number" min={total} step="0.01" value={amountReceived} onChange={event => setAmountReceived(event.target.value)} placeholder={total.toFixed(2)}/><small>Leave empty for the exact total.</small></label></fieldset>
          <dl><div><dt>Subtotal</dt><dd>{money.format(subtotal)}</dd></div>{discountNumber > 0 && <div><dt>Discount</dt><dd>−{money.format(discountNumber)}</dd></div>}<div className={styles.total}><dt>Total due</dt><dd>{money.format(total)}</dd></div><div><dt>Change to return</dt><dd>{money.format(Math.max(0, received - total))}</dd></div></dl>
          {error && mobileCart && <p className={styles.error} role="alert">{error}</p>}
          <button type="submit" className={styles.pay} disabled={!lines.length || submitting || loading}>{submitting ? "Completing sale…" : "Complete sale · " + money.format(total)}<ArrowRight size={17} aria-hidden="true"/></button>
        </form>
      </aside>
    </div>
