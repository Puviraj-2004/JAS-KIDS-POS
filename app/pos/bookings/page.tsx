...
export default async function BookingsPage({ searchParams }: { searchParams: { q?: string; type?: string; state?: string; branch_id?: string } }) {
  ...
  const selectedBranchId = isSuperAdmin && typeof searchParams.branch_id === "string" ? searchParams.branch_id : staff.branch_id ?? "";
  const bookingType = Object.values(BookingType).includes(searchParams.type as BookingType) ? searchParams.type as BookingType : undefined;
  const where: Prisma.BookingWhereInput = {
    ...(selectedBranchId ? { branch_id: selectedBranchId } : {}),
    ...(bookingType ? { booking_type: bookingType } : {}),
    ...(searchParams.state === "checked_in" ? { checkin: { isNot: null } } : searchParams.state === "pending" ? { checkin: null } : {}),
  ...
  const todayCount = bookings.filter((booking) => booking.booking_date.toISOString().slice(0, 10) === todayKey).length;
  ...
  const slotRows = Object.values(bookings.reduce<Record<string, { key: string; date: Date; start: string; end: string; type: BookingType; count: number }>>((result, booking) => {
    const key = `${booking.booking_date.toISOString().slice(0, 10)}-${booking.start_time}-${booking.end_time}-${booking.booking_type}`;
  ...
  return <main className={styles.page}><div className={styles.shell}>
    ...
    <nav className="record-tabs" aria-label="History type"><Link href="/pos/sales">Sales</Link><Link href="/pos/bookings" aria-current="page">Bookings</Link></nav>
    <form className={styles.filters}>{isSuperAdmin && <select name="branch_id" defaultValue={selectedBranchId} aria-label="Branch"><option value="">All branches</option>{branches.map(branch => <option key={branch.id} value={branch.id}>{branch.name}</option>)}</select>}<input name="q" defaultValue={q} aria-label="Search bookings" placeholder="Reference, parent, or QR code" /><select name="type" defaultValue={searchParams.type ?? ""} aria-label="Booking type"><option value="">All booking types</option>{bookingTypeOptions.map(option => <option key={option.value} value={option.value}>{option.label}</option>)}</select><select name="state" defaultValue={searchParams.state ?? ""} aria-label="Check-in status"><option value="">All statuses</option><option value="pending">Awaiting check-in</option><option value="checked_in">Checked in</option></select><button type="submit">Filter</button></form>
